package effect.volumetrics
{
	...
	import volumetrics.VolumetricPointLight;
	
	public class PointLightDemo extends BaseTest
	{
		private var fx:VolumetricPointLight;
		private var grid:Grid = new Grid;
		private var sun:SunIcon = new SunIcon;
		
		public function PointLightDemo(stage:Stage)
		{
			super(stage);
		}
	...
	private function init(e:Event = null):void {
		...
		for (var i:int = 0; i < 6; i++ ) {
			for (var j:int = 0; j < 6; j++ ) {
				...
				Arrays.push(_point)
				var tes:Sprite = addChild(new Sprite()) as Sprite;
				tes.graphics.beginFill(0x101010);
				tes.graphics.drawCircle(0, 0, 20);
				tes.graphics.endFill();
				SQ.push(tes)
			}
		}
