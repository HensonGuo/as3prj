package paintglow
{
	...
	public class PantsGlow extends Sprite
	{
		...
		private static function rotateX(v:Vector2D, r:Number):Number
		{
			var y:Number = v.y;
			if (y<0) y=-y;
		...
		private function drawFin(g:Graphics, g2:Graphics, g3:Graphics, idx:int, ofs:Number):void
		{
			...
			var vP:Vector2D = new Vector2D(x, y);
			var vO:Vector2D = new Vector2D(ox, oy);
			var vMid:Vector2D = Vector2D.makeAtoB(vP, vO);
			...
			var vLeft:Vector2D = vMid.clone().normalize().smul(19).left();
			var vRight:Vector2D = vLeft.clone().smul(-1);

			var gw:Number = vMid.length;
